...
export function TimelineControl({ periods, value, onChange }: TimelineControlProps) {
  ...
  return <fieldset className="timeline-control">
    ...
    <div className="timeline-toolbar">
      ...
      <output aria-live="polite">{formatPeriodLabel(value)}</output>
    ...
    {orderedPeriods.length <= 14 ? <div className="timeline-options" role="group" aria-label="Cambia periodo">
      {orderedPeriods.map((period) => <button type="button" key={period} onClick={() => onChange(period)} aria-pressed={period === value}>{formatPeriodLabel(period)}</button>)}
      ...
      <div aria-hidden="true"><span>{formatPeriodLabel(orderedPeriods[0])}</span><span>{formatPeriodLabel(orderedPeriods[Math.floor(orderedPeriods.length / 2)])}</span><span>{formatPeriodLabel(orderedPeriods.at(-1))}</span></div>
    </div>}
  </fieldset>;
}
