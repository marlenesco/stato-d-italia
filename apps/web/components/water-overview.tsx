...
export function WaterOverview({ overview }: { overview: WaterOverview }) {
  ...
  return <>
    <section className="water-hero"><div className="water-hero-title"><p className="eyebrow">BIGBANG 10.0 · serie 1951–2025</p><h1>Acqua, anno per anno.</h1></div><div className="water-hero-copy"><p>Stime ufficiali modellistiche per Italia e Regioni. Una mappa per ogni anno; nessuna interpolazione.</p><Link className="water-primary-link" href={atlasHref("water_total_precipitation_mm", "2025")}>Apri atlante 2025 <span aria-hidden="true">→</span></Link></div>{precipitation.length > 1 && <CountryTrend series={precipitation} />}</section>
    ...
    <section className="water-metric-grid" aria-label="Ultime osservazioni nazionali">{overview.countryProfile.latestObservations.map((observation) => <Link key={observation.metricId} href={atlasHref(observation.metricId, observation.periodEnd.slice(0, 4))}><span>{labels[observation.metricId] ?? observation.metricId}</span><strong>{format(observation.value, observation.unit)}</strong><small>{observation.periodEnd.slice(0, 4)} · Italia</small></Link>)}</section>
  </>;
}
