...
import { profileUnavailableCopy, shouldRenderProfileLink, shouldRenderTerritorySeries } from "../lib/explorer-rendering";
...
export function waterMapSeriesProps(features: ExplorerFeatures) {
  return shouldRenderTerritorySeries(features.territorySeries) ? { comparisonStatus: features.comparison.status } : undefined;
}

export function WaterMap({ option, metricLabel, geometryUrl, territoryLevel, derived, features, currentTerritoryIds, selectedTerritoryId, seriesOptions, onTerritorySelect }: { option: MapOption; metricLabel: string; geometryUrl?: string; territoryLevel: "region" | "province"; derived: boolean; features: ExplorerFeatures; currentTerritoryIds?: string[]; selectedTerritoryId?: string; seriesOptions?: MapOption[]; onTerritorySelect?: (territoryId: string, name?: string) => void }) {
