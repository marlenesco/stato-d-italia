...
export function WaterMap({ option, metricLabel, geometryUrl }: { option: MapOption; metricLabel: string; geometryUrl?: string }) {
  ...
  useEffect(() => {
    ...
    async function createMap() {
      try {
        ...
        map = new maplibregl.Map({ container: mapTarget, style: { version: 8, sources: { territories: { type: "vector", url: `pmtiles://${pmtilesUrl}`, promoteId: "territory_id" } }, layers: [{ id: "water-background", type: "background", paint: { "background-color": "#eef3ee" } }, { id: "water-fill", type: "fill", source: "territories", "source-layer": "territories", paint: { "fill-color": colorExpression(0, 1), "fill-opacity": 0.86 } }, { id: "water-line", type: "line", source: "territories", "source-layer": "territories", paint: { "line-color": "#fffdf7", "line-width": 0.45 } }] }, center: [12.5, 42.8], zoom: 5 });
        const activeMap = map;
