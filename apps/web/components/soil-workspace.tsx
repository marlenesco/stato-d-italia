  ...
  return <section className={`soil-site-layout explorer-layout ${config.domainClass ?? ""}`} aria-label={`Esplorazione ${themeLabel}`}>
    <div className="soil-site-content">
      ...
      <section id="mappa" className="soil-workspace map-workspace-v2" tabIndex={-1} aria-label={`Mappa ${title.toLowerCase()}`}>
        {selected && model.features.timeline.status === "available" && <TimelineControl periods={periods} value={selected.periodKey} onChange={(period) => update({ period })} />}
        {selected ? <SoilMap option={selected} metricLabel={labels[selected.metricId] ?? selected.metricId} geometryUrl={model.geometryUrl} rankingUrl={model.rankingUrl} features={model.features} selectedTerritoryId={territory?.id} seriesOptions={model.features.territorySeries.status === "available" ? available : undefined} seriesStatusNote={guide?.seriesStatusNote ?? config.seriesStatusNote} colorRamp={config.colorRamp} comparisonNote={config.comparisonNote} sharedTemporalScale={config.sharedTemporalScale} currentTerritoryIds={data.currentTerritoryIds?.[selected.level]} onTerritorySelect={selectTerritory} /> : <p role="alert">Combinazione non disponibile nella release attiva.</p>}
